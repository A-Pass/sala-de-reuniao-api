package apass.saladereuniao.controller;
...
import apass.saladereuniao.exception.ResourceNotFoundException;
import apass.saladereuniao.model.Room;
import apass.saladereuniao.repository.RoomRepository;
...
public class RoomController {
	...
	public ResponseEntity<Room> getRoomById(@PathVariable long id) throws ResourceNotFoundException {
		var room = this.roomRepository.findById(id)
				.orElseThrow(() -> new ResourceNotFoundException("Room not foound " + id));
		return ResponseEntity.ok().body(room);
	}

	@PostMapping("/")
	@ResponseStatus(code = HttpStatus.CREATED)
	public Room createRom(@Valid @RequestBody Room room) {
	...
	public Map<String, Boolean> deleteRoom(@PathVariable(value = "id") Long id) throws ResourceNotFoundException {
		Room room = this.roomRepository.findById(id).orElseThrow(()-> new ResourceNotFoundException("Room not found for this id:" + id));
